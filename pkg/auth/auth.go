...
import (
	...
	"github.com/dhenkes/gobmarks/pkg/gobmarks"
...
var _ gobmarks.AuthService = (*AuthService)(nil)
...
func GenerateRandomBytes(n int) ([]byte, error) {
	if n < -1 {
		return nil, gobmarks.NewError(gobmarks.EINTERNAL, "Length must be a positive int.")
	}
...
func (s *AuthService) HashPassword(password string, salt string) (string, error) {
	if password == "" {
		return "", gobmarks.NewError(gobmarks.EINVALID, "Password required.")
	}

	if salt == "" {
		return "", gobmarks.NewError(gobmarks.EINVALID, "Salt required.")
	}
...
func (s *AuthService) VerifyPassword(password string, key string) error {
	...
	if key == "" {
		return gobmarks.NewError(gobmarks.EINVALID, "Argon2 key required.")
	}

	decodedKey := strings.Split(key, "$")
	if len(decodedKey) != 6 {
		return gobmarks.NewError(gobmarks.EINVALID, "Decoded key wrong length.")
	}
	...
	if p.Version != argon2.Version {
		return gobmarks.NewError(gobmarks.EINVALID, "Argon version mismatch.")
	}
	...
	if subtle.ConstantTimeCompare(hash, control) == 1 {
		...
		return gobmarks.NewError(gobmarks.EINVALID, "Hash not equal password.")
	}
}
